...
- **Playwright** (config: `claude mcp add --transport stdio playwright -- npx @playwright/mcp`) — only if invoices must be downloaded from a vendor or accounting portal first.

If a needed server isn't connected, tell the user exactly which one to add and wait.
If the task must drive a native desktop app (a scanner or desktop accounting client), enable **computer use** so the assistant can operate the app directly.
