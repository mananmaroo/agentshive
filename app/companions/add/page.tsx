...
export default function AddCompanion() {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    if (!user) {
      setError('Please log in to add a companion.');
      return;
    }
    if (!terms) {
      setError('Please confirm the companion is safe and yours to share.');
      return;
    }
    ...
    setLoading(true);
    try {
      // 'Companion' is always included so submissions appear in the /companions
      // community grid; any extra category the user picks is kept alongside it.
      const categories = ['Companion', ...(category ? [category] : [])].filter(
      ...
      setError(err.message || 'Failed to add companion. Please try again.');
      setLoading(false);
    }
  };
  ...
  const formats = [
    {
      id: 'claude-md',
      name: 'Claude.md',
      description: 'Plain text file with companion instructions',
      icon: FileText,
      color: 'blue',
      recommended: true,
    },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <nav className="bg-slate-900/50 border-b border-slate-700 backdrop-blur-sm sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href="/companions" className="text-slate-300 hover:text-white transition text-sm">
              Companions
            </Link>
      ...
      <div className="max-w-3xl mx-auto px-4 pt-8">
        <Link href="/companions" className="inline-flex items-center gap-2 text-blue-500 hover:text-blue-400 font-semibold text-sm mb-8">
      ...
      <div className="max-w-3xl mx-auto px-4 pb-20">
        <div className="mb-12">
          <div className="inline-flex items-center gap-2 bg-blue-500/10 text-blue-300 text-xs font-medium px-3 py-1 rounded-full mb-4">
            <Terminal className="w-3.5 h-3.5" />
            Terminal Edition
          </div>
          <h1 className="text-4xl font-bold text-white mb-2">Add a Companion</h1>
          <p className="text-lg text-slate-400">
            Share a hands-on companion that runs in Claude Code. It will be listed under the
            Companions community grid.
          </p>
        </div>
        ...
        <form onSubmit={handleUpload} className="space-y-6">
          {/* Companion Name */}
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-2">Companion Name *</label>
            <input
              type="text"
              placeholder="e.g., Personal Assistant, Document Visualizer"
              value={agentName}
          ...
          <div>
            ...
            <textarea
              placeholder="Describe what your companion does, the tools/MCP it needs, and how it runs in Claude Code"
              value={description}
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mb-2">
              ...
              <span className="font-semibold text-slate-300">Companion</span>. Optionally add a
            ...
            >
              <option value="">Companion only</option>
              {categories.map((cat) => (
          ...
          <div>
            ...
            <input
              type="text"
              placeholder="e.g., mcp, claude-code, assistant, terminal (comma-separated)"
              value={tags}
          ...
          <div className="bg-slate-800/30 border border-slate-700 rounded-lg p-4">
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              <span className="text-sm text-slate-300">
                I confirm that this companion doesn't contain malicious code and I have the right to share it.
              </span>
            </label>
          </div>
          ...
          {!authLoading && !user && (
            <div className="bg-amber-500/10 border border-amber-500/30 text-amber-200 px-4 py-3 rounded-lg text-sm">
              You need to be logged in to add a companion.{' '}
        ...
        <div className="mt-16 border border-slate-800 rounded-lg p-8">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h3 className="text-xl font-semibold text-white">Read this before you add a companion</h3>
          </div>
