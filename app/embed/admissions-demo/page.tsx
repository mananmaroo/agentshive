...
export default function AdmissionsDemo() {
  ...
  const submitMessage = async (value: string) => {
    ...
    setSending(true);
    try {
      const response = await fetch('/api/demo/aarya/respond', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ widgetKey: WIDGET_KEY, question, conversationId: conversationId.current }) });
  ...
  return <main className="flex min-h-screen items-center justify-center bg-transparent p-3 text-slate-100">
    <section className="flex h-[620px] w-full max-w-md flex-col overflow-hidden rounded-2xl border border-slate-700 bg-slate-950 shadow-2xl">
      <header className="flex items-center justify-between border-b border-slate-800 bg-slate-900 px-5 py-4"><div className="flex items-center gap-3"><div className="rounded-xl bg-emerald-500/10 p-2 text-emerald-300"><Sparkles className="h-5 w-5" /></div><div><h1 className="font-bold">Aarya · Admissions</h1><p className="text-xs text-emerald-300">Guided public demo · Fictional data</p></div></div><Volume2 className="h-5 w-5 text-slate-500" /></header>
