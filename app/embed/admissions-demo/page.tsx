...
export default function AdmissionsDemo() {
  const [messages, setMessages] = useState<Message[]>([{ id: 1, sender: 'aarya', text: 'Hello! I’m Aarya. This access-gated preview will use approved Aarohan information only after the demo workspace is legitimately activated. Until then, I safely request human review.' }]);
  ...
  return <main className="flex min-h-screen items-center justify-center bg-transparent p-3 text-slate-100">
    <section className="flex h-[620px] w-full max-w-md flex-col overflow-hidden rounded-2xl border border-slate-700 bg-slate-950 shadow-2xl">
      <header className="flex items-center justify-between border-b border-slate-800 bg-slate-900 px-5 py-4"><div className="flex items-center gap-3"><div className="rounded-xl bg-emerald-500/10 p-2 text-emerald-300"><Sparkles className="h-5 w-5" /></div><div><h1 className="font-bold">Aarya · Admissions</h1><p className="text-xs text-emerald-300">Access-gated preview · Inactive</p></div></div><Volume2 className="h-5 w-5 text-slate-500" /></header>
