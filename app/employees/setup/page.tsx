...
export default function EmployeeSetupPage() {
  ...
  if (loading || accessChecking) return <main className="min-h-screen bg-slate-950 p-10 text-slate-400">Checking business access…</main>;
  ...
  if (!accessAllowed) return <main className="flex min-h-screen items-center justify-center bg-slate-950 px-4 text-white"><section className="max-w-lg rounded-2xl border border-rose-500/30 bg-slate-900 p-8 text-center"><ShieldCheck className="mx-auto h-10 w-10 text-rose-300" /><h1 className="mt-5 text-2xl font-bold">Business access unavailable</h1><p role="alert" className="mt-3 text-slate-300">{error || 'This workspace is inactive, unpaid, revoked, or expired.'}</p><Link href="/employees" className="mt-6 inline-block rounded-lg border border-slate-700 px-5 py-3 font-semibold">Return to AgentsHive Business</Link></section></main>;
